          ...
          <form class="post_form form" action="{{ route('post.update', $items->id )}}" method="post" enctype="multipart/form-data">
            ...
            <div class="card -form">
              ...
              <div class="input_wrap -margin">
                ...
                <div class="add_images_wrap">
                  <div class="add_image_input">
                    <input type="file" name="image_01" class="-secondary" required id="addImages01" accept="image/*" value="{{ $items->temp_path_01 }}">
                    <figure>
                  ...
                  <div class="add_image_input">
                    <input type="file" name="image_02" class="-secondary" id="addImages02" accept="image/*" value="{{ $items->temp_path_02 }}">
                    <figure>
                  ...
                  <div class="add_image_input">
                    <input type="file" name="image_03" class="-secondary" id="addImages03" accept="image/*" value="{{ $items->temp_path_03 }}">
                    <figure>
              ...
              <div class="input_wrap">
                ...
                <input type="text" name="weight" class="-secondary @error('weight') is-invalid @enderror" required placeholder="500g→0.5 / 1kg→1" id="weight" value="{{ $items->weight }}">
                <p class="alert -top">＊半角数字のみ。単位は省略してください。</p>
              </div>
              <div class="input_wrap">
                <label for="price" class="label">参考価格（大体の税抜き価格で結構です）</label>
                <input type="text" name="price" class="-secondary @error('price') is-invalid @enderror" required placeholder="3000円→3000" id="price" value="{{ $items->price }}">
                <p class="alert -top">＊半角数字のみ。単位は省略してください。</p>
              </div>
              <div class="input_wrap">
                <label for="per_protein" class="label">１食あたりのタンパク質含有量</label>
                <input type="text" name="per_protein" class="-secondary @error('per_protein') is-invalid @enderror" required placeholder="20g→20" id="per_protein" value="{{ $items->per_protein }}">
                <p class="alert -top">＊半角数字のみ。単位は省略してください。</p>
              </div>
              <div class="radio_wrap">
                <p>日本のメーカーですか？</p>
                <input type="radio" name="made" value="0" class="-secondary @error('made') is-invalid @enderror" required id="madejapan" {{ $items->made == "0" ? 'checked' : '' }}>
                <label for="madejapan" class="label">はい</label>
                <input type="radio" name="made" value="1" class="-secondary @error('made') is-invalid @enderror" required id="madeother" {{ $items->made == "1" ? 'checked' : '' }}>
              ...
              <div class="radio_wrap">
                ...
                <input type="radio" name="type" value="0" class="-secondary @error('type') is-invalid @enderror" required id="whey" {{ $items->type == "0" ? 'checked' : '' }}>
                <label for="whey" class="label">ホエイ</label>
                <input type="radio" name="type" value="1" class="-secondary @error('type') is-invalid @enderror" required id="soy" {{ $items->type == "1" ? 'checked' : '' }}>
                <label for="soy" class="label">SOY</label>
                <input type="radio" name="type" value="2" class="-secondary @error('type') is-invalid @enderror" required id="casein" {{ $items->type == "2" ? 'checked' : '' }}>
                <label for="casein" class="label">カゼイン</label>
                <input type="radio" name="type" value="3" class="-secondary @error('type') is-invalid @enderror" required id="typeother" {{ $items->type == "3" ? 'checked' : '' }}>
