...
class PostController extends Controller
{
    ...
    public function update(EditPostForm $request, $id) {
        ...
        // 画像を追加しない場合
        if ($request->submit == 'noImage') {

          $path_01 = null;
          $path_02 = null;
          $path_03 = null;

          if ( is_null($old_path_01) == false ) {
            Storage::disk('s3')->delete($old_path_01, 'public');
          }
          if ( is_null($old_path_02) == false ) {
            Storage::disk('s3')->delete($old_path_02, 'public');
          }
          if ( is_null($old_path_03) == false ) {
    ...
    public function noImageStore(EditPostForm $request) {
      $post = new Post;
